import { IResData, Title, useSyncHttpCient, Notify, warning, error } from '@vntgcorp/vntg-wdk-client';
...
const EDU000E04 = (props: Props) => {
  ...
  const onSave = () => {
    ...
    try {
      // 교육계획건수가 있는지 체크
      const eduPlanCnt = saveData[0].edu_plan_cnt;
      if (eduPlanCnt > 0) {
        warning('교육계획/실적 등록에 등록된 데이터가 있습니다. 수정/삭제 불가능합니다.');
        return [];
      }
    } catch (err) {
      error(err);
